...
async def start(event):
    ...
    SENDER = sender.id
    text = "Hola, Yo puedo hacer operaciones CRUD en la base de datos local operations inside a local database"
...
async def insert(event):
    try:
        ...
        value = list_of_words[3] # the four (4) items are the value
        ...
        params = (product, quantity, value, dt_string)
        sql_command = "INSERT INTO products VALUES (NULL, ?, ?, ?,?);" # the initial NULL is for the AUTOINCREMENT id inside the table
        ...
        if crsr.rowcount < 1:
            ...
            text = "Productos insertados correctamente"
            await client.send_message(SENDER, text, parse_mode='html')

        
    except Exception as e: 
        print(e)
        await client.send_message(SENDER, "<b>Conversación Finalizada✔️</b>", parse_mode='html')
        return
...
def create_message_select_query(ans):
    ...
    message = "<b>Recibida 📖 </b> Información sobre los productos:\n\n"+text
    return message
...
async def select(event):
    try:
        ...
        print(e)
        await client.send_message(SENDER, "<b>Conversación Terminada✔️</b>", parse_mode='html')
        return
...
async def update(event):
    try:
        ...
        new_value = list_of_words[4] # fifth (4) item is the value
        dt_string = datetime.now().strftime("%d/%m/%Y") # We cat the new date

        # create the tuple with all the params interted by the user
        params = (id, new_product, new_quantity, new_value,dt_string, id)

        # Create the UPDATE query, we are updating the product with a specific id so we must put the WHERE clause
        sql_command="UPDATE products SET id=?, product=?, quantity=?, value=? , date=? WHERE id =?" 
        crsr.execute(sql_command, params) # Execute the query
        conn.commit() # Commit the changes

        # If at least 1 row is affected by the query we send a specific message
        if crsr.rowcount < 1:
            text = "EL producto con el id {} no existe".format(id)
            await client.send_message(SENDER, text, parse_mode='html')
        else:
            text = "El producto con el id {} se actualizó correctamente".format(id)
            await client.send_message(SENDER, text, parse_mode='html')
...
async def delete(event):
    try:
        ...
        sql_command = "DELETE FROM products WHERE id = (?);"
        ans = crsr.execute(sql_command, (id,))
        conn.commit()
        
        # If at least 1 row is affected by the query we send a specific message
        if ans.rowcount < 1:
            text = "El producto con el id {} no se encuentra".format(id)
            await client.send_message(SENDER, text, parse_mode='html')
        else:
            text = "El producto con el id {} se eliminó correctamente".format(id)
            await client.send_message(SENDER, text, parse_mode='html')
